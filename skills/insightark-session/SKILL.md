...
- `credits.usage` — peek InsightArk MCP credit balance for an organization (requires `orgId`; does not consume credits)
...
3. Call `credits.usage` with `orgId` when the user asks how many InsightArk MCP credits remain, or after `429 error/credit-exhausted` to report `secondsUntilNextCredit`. This call does not consume credits (only org RPM applies).
