...
5. **Credits / 429** — Before calling a credit-consuming tool, state the documented catalog estimate when useful. Usage questions belong on `credits_usage` (`usage.total` for this client today when `from`/`to` are omitted). Tools do not return a per-call credit receipt; do not treat tool JSON as one. Never infer consumption from `credits_usage` monthly `remaining` differences, especially across concurrent calls. On `429` / credit exhaustion, do not retry loops that burn more credits; use `credits_usage` only to inspect remaining balance or stop and report. Do not conflate InsightArk MCP credits with Console AI 點數 or messaging quota.
