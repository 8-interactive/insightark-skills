...
- **Terminal** — report `deliveryOutcome`: `delivered`, `done_with_failures`, `no_recipients`, or `delivery_incomplete`. For failures, use only aggregate `failureDiagnostics`, its allowlisted code/category/retryability, `unclassifiedFailedCount`, and `taskId` as the support reference. Never expose or infer recipient details or raw provider payloads.
