...
if [ -z "$conversation_id" ]; then
  printf 'Missing required option: --conversation-id\n' >&2
  exit 1
fi
...
[ -n "$order" ] && query_parts+=("$(s8_query_pair order "$order")")
...
s8_api_request GET "/developer/v1/conversations/${conversation_id}/messages${query_string}" "" "$response_file" "$status_file"
