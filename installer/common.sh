...
super8_count_bundle_skills() {
  ...
  shopt -s nullglob
  for entry in "$bundle_dir"/super8-studio-*/; do
    [ -d "$entry" ] || continue
    count=$((count + 1))
  done
  shopt -u nullglob
...
super8_print_interactive_header() {
  ...
  if [ "$mode" = "install" ]; then
    printf '%s\n' '  Super 8 Studio API Skills — Installer' >&2
  else
    printf '%s\n' '  Super 8 Studio API Skills — Uninstaller' >&2
  fi
  printf '%s\n' '============================================================' >&2
  printf '\n' >&2
  ...
  if [ "$mode" = "install" ]; then
    ...
    printf 'run ./setup-env.sh to set up ~/.super8-studio.env.\n' >&2
  else
    printf 'This script removes Super 8 Studio API skills previously\n' >&2
...
super8_print_install_plan() {
  ...
  printf '%s\n' '============================================================' >&2
  printf '\n' >&2
  printf 'Super 8 Studio API skills will be installed to:\n' >&2
  printf '\n' >&2
...
super8_prompt_agents() {
  ...
  if [ "$mode" = "install" ]; then
    ...
    printf '\n' >&2
    printf 'Select agents to remove Super 8 Studio API skills from:\n' >&2
  fi
  printf '\n' >&2
  num=1
...
super8_list_bundle_dirs() {
  ...
  local entry
  shopt -s nullglob
  for entry in "$bundle_dir"/super8-studio-*/ "$bundle_dir"/_super8-studio-*/; do
    [ -d "$entry" ] || continue
    basename "$entry"
  done
  shopt -u nullglob
...
super8_remove_bundle_dirs() {
  ...
  local entry
  shopt -s nullglob
  for entry in "$target_dir"/super8-studio-*/ "$target_dir"/_super8-studio-*/; do
    [ -d "$entry" ] || continue
    rm -rf "$entry"
  done
  shopt -u nullglob
...
super8_build_console_token_url() {
  ...
  local label="${2:-super8-studio-skills}"
