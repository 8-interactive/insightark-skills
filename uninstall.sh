...
for target in "${remove_targets[@]}"; do
  if [ -d "$target" ]; then
    ...
    printf 'Removed Super 8 Studio API skills from %s\n' "$(super8_format_display_path "$target")"
    if [ -f "${target%/}/.super8-studio.env" ]; then
      rm -f "${target%/}/.super8-studio.env"
      printf 'Removed %s\n' "$(super8_format_display_path "${target%/}/.super8-studio.env")"
    fi
  else
