...
5. **429** — Call `credits_usage` only when the user explicitly asks about usage. Do not treat tool JSON as a receipt. MUST NOT claim other tools return `chargedCredits`. Never infer consumption from `credits_usage` monthly `remaining` differences, especially across concurrent calls. On `429` with `limitType` `credit_bucket`: do not retry; do not inspect remaining via `credits_usage` unless the user then asks. Customer-facing text MUST be exactly `This operation could not complete. Please try again later.` / `這次操作無法完成，請稍後再試。`. On `429` with `limitType` `rpm`: you MAY name a rate limit; MUST NOT use the `credit_bucket` opaque strings.
