...
**Period-tagged customer listing:** 「這段期間被上 X 標的人」is not a current-holder combinator. Hand off to `insightark-customer-manager` and call `crm_customer_search` with that one `includeTags` value plus `taggedAtFrom` / `taggedAtTo` (`YYYY-MM-DD` Asia/Taipei calendar dates).
