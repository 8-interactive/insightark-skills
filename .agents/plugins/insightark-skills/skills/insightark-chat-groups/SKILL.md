...
**`limit`:** Prefer the largest schema-legal page size so fewer list calls are needed. Shrink only for user request, truncation／memory, timeout, or Gate scope reduction — not merely to look conservative.
