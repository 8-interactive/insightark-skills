...
| Quotas (訊息則數 / 旅程次數) | `limits.message`（總發送則數上限）與 `limits.per_customer`（每位顧客可進入旅程次數）**皆必填**；皆須為非負整數，**或** `per_customer` 使用字串 **`onceByDay`** — 須由客戶明確選擇。 |
| Messenger tag | For Meta channels, confirm `fbTag`; **do not** assume `NO_TAG` without asking. |
| Off-hours / OOS (休眠) | **預設關閉**：除非客戶明確要開啟，否則使用 `oos: { "enabled": false, ... }` 或客戶明確選擇的 `null`；**若 `enabled: true`**，必須與客戶確認並填寫 `hour`、`minute`、`duration`（秒）。 |
...
6. Run **`ma_procedure_validate.sh`**; if `data.valid === false`, parse **`data.errors`** (`path`, `code`, `message`) and **逐條向客戶說明缺什麼**；僅在客戶同意後改 JSON。
7. After a successful validate, run **`ma_procedure_create.sh`**；若 HTTP **400**，讀 **`data.errors`**（通常與 validate 同形），用 **`path` + `message`** 請客戶補齊。Cap at **three** validate-fix rounds.
8. **Do not** copy names, times, keywords, or copy from sample JSON onto a real customer “to save time.”

**讀取校驗結果**

- Validate：`jq -r '.data.errors[]? | "\(.path // "(root)"): \(.message)"' response.json`
- Create 失敗：`success: false` 時同樣檢查 **`data.errors`**。
